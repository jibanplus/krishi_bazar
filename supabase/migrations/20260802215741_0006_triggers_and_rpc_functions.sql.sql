...
BEGIN
  ...
  -- Credit to bonus section only with 20x wagering requirement
  UPDATE public.wallets SET 
    bonus_wagering_locked = bonus_wagering_locked + v_bonus,
    wagering_required = wagering_required + (v_bonus * 20)
  WHERE user_id = p_user_id;
  UPDATE public.profiles SET bonus_balance = bonus_balance + v_bonus, signup_bonus_credited = true WHERE id = p_user_id;

  INSERT INTO public.transactions (user_id, type, amount, status, description)
  VALUES (p_user_id, 'bonus', v_bonus, 'approved', 'সাইন আপ বোনাস / Signup Bonus (20x wagering required)');

  INSERT INTO public.notifications (user_id, title, message, type)
  VALUES (p_user_id, 'স্বাগত বোনাস / Welcome Bonus', '₹' || v_bonus || ' সাইন আপ বোনাস আপনার বোনাস সেকশনে যোগ হয়েছে। 20x ট্রেড সম্পন করতে হবে।', 'bonus');
